...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //First inject itunesService into your controller.
    $scope.getSongData = function(){
      itunesService.getMusic($scope.artist)
        .then(
          function(response){
            console.log(response);
            $scope.songData = parseData(response);
          }
        )
        .catch(
          function(err){
  ...
  */
  //the iTunes API is going to give you a lot more details than ng-grid wants. Create a new array and then loop through the iTunes data pushing into your new array objects that look like the above data.
    ...
    //Code here
  // function parseData (songArr){
  //   var newArr = [];
  //   for(var i = 0; i < songArr.length; i++){
  //     var newObj = {
  //       Play: songArr[i].previewUrl,
  //       Artist: songArr[i].artistName
  //     }
  //     newArr.push(newObj)
  //   }
  //   return newArr;
  ...
  function parseData(songData){
    ...
    for (var i = 0; i < songData.length; i++){
      var song = {
        ...
        CollectionPrice: songData[i].trackPrice
      }
      songArr.push(song);
    }
    return songArr;
